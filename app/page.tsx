...
import { Shirt, CalendarCheck, TrendingUp, Store, Sprout, type LucideIcon } from "lucide-react"
...
type Tab = "wardrobe" | "tracker" | "impact" | "marketplace"

const tabs: { id: Tab; label: string; icon: LucideIcon }[] = [
  { id: "wardrobe", label: "Wardrobe", icon: Shirt },
  { id: "tracker", label: "Daily Tracker", icon: CalendarCheck },
  { id: "impact", label: "Savings Impact", icon: TrendingUp },
  { id: "marketplace", label: "Marketplace", icon: Store },
]

export default function Page() {
  const [tab, setTab] = useState<Tab>("wardrobe")
  // Entire UI is driven by local React state.
    ...
    <main className="relative min-h-dvh bg-background pb-32">
      ...
      <header className="mx-auto flex max-w-5xl items-center justify-between px-6 pt-8">
        ...
        <div className="text-right">
          <p className="text-sm font-medium text-foreground">{items.length} pieces</p>
          <p className="text-xs text-muted-foreground">tracked mindfully</p>
        </div>
      </header>
          ...
          {tabs.map(({ id, label, icon: Icon }) => {
            ...
            return (
              <button
                ...
                onClick={() => setTab(id)}
                className={cn(
                  "relative flex items-center gap-2 rounded-full px-3.5 py-2.5 text-sm font-medium transition-colors sm:px-4",
